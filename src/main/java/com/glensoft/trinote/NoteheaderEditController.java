...
public class NoteheaderEditController  implements Initializable {
	...
	private Boolean Validate()
	{
		if (this.txtName.getLength() == 0)
		{
			...
			alert.setHeaderText("There was an error in the Header");
	...
	private void handleOK() {
		if (this._isModelDirty || (this._model.getId() == 0l)) {
			...
			{
				Context.getInstance().getProvider().putNoteheader(this._model);
			}
			else
			{
				Context.getInstance().getProvider().postNoteheader(this._model);
			}
		}
